...
WORKDIR /src
COPY ["recommendo-app-api.csproj", "recommendo-app-api/"]
RUN dotnet restore "recommendo-app-api/recommendo-app-api.csproj"

WORKDIR "/src/recommendo-app-api"
COPY . .
RUN dotnet build "recommendo-app-api.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "recommendo-app-api.csproj" \
...
Execute unit tests
```
    cd "recommendo-app-api/test/recommendo-app-api.Tests"
    dotnet test
```

Deploy application
```
    cd "recommendo-app-api/src/recommendo-app-api"
